...
export const EmployeeDetailsPage: React.FC = () => {
  ...
  const handleDocUploadSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // 1. Upload to S3 (via our backend upload route)
  ...
  const handleAddVersionSubmit = async (docId: string) => {
    ...
    try {
      ...
      // 2. Add version record in database
  ...
  return (
    <div className="space-y-8 text-left animate-in fade-in duration-300 pb-12">
      ...
      <div className="w-full space-y-6">
        ...
        {/* TAB 6: DOCUMENTS (S3 Uploader + List with Version Updates) */}
        {activeTab === 'DOCUMENTS' && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-in fade-in duration-300">
            {/* Upload form */}
            <div className="space-y-6">
              <Card className="p-6 border border-border shadow-md space-y-6">
                <h3 className="text-lg font-bold text-foreground flex items-center gap-2 border-b border-border pb-3">
                  <Upload className="w-5 h-5 text-primary" /> Upload Employee Document
                </h3>
                <form onSubmit={handleDocUploadSubmit} className="space-y-4">
                  <Input
                    label="Document Display Name *"
                    placeholder="e.g. Passport, Employment Contract"
                    value={docName}
                  ...
                  <div className="flex flex-col gap-1.5 text-left">
                    ...
                    >
                      <option value="ID_PROOF">ID Proof (PAN, Aadhaar, etc.)</option>
                      <option value="CONTRACT">Employment Contract</option>
                      <option value="PASSPORT">Passport Details</option>
                      <option value="VISA">Visa & Work Permits</option>
                      <option value="CERTIFICATE">Certificates (Degree, Experience)</option>
