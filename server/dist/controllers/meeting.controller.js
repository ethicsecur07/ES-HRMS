...
exports.joinMeetingRedirect = exports.deleteMeeting = exports.updateMeeting = exports.getMeetingById = exports.getMeetings = exports.scheduleMeeting = void 0;
const nodemailer_1 = __importDefault(require("nodemailer"));
const Meeting_js_1 = require("../models/Meeting.js");
const teamsMeeting_service_js_1 = require("../services/teamsMeeting.service.js");
 ...
 */
const getOrgMicrosoftCredentials = async (orgId) => {
    if (!orgId)
        return undefined;
    try {
        ...
        logger_js_1.logger.error('[MeetingController] Failed to query OrganizationAuthConfig', { error: err.message });
    }
    return undefined;
};
/**
...
const scheduleMeeting = async (req, res) => {
    try {
        ...
        const teamsMeeting = await (0, teamsMeeting_service_js_1.createTeamsMeeting)({
            subject: title,
            startDateTime,
            endDateTime,
        ...
        logger_js_1.logger.info(`[MeetingController] Meeting scheduled: ${title} (${meetingType})`);
        res.status(201).json({
            success: true,
        ...
        logger_js_1.logger.error('[MeetingController] scheduleMeeting error', { error: error.message });
...
const getMeetings = async (req, res) => {
    try {
        ...
        const [meetings, total] = await Promise.all([
            Meeting_js_1.Meeting.find(filter)
            ...
            Meeting_js_1.Meeting.countDocuments(filter),
        ]);
        res.status(200).json({
            success: true,
            meetings,
        ...
        logger_js_1.logger.error('[MeetingController] getMeetings error', { error: error.message });
        res.status(500).json({ success: false, message: error.message });
    }
};
...
const getMeetingById = async (req, res) => {
    try {
        const meeting = await Meeting_js_1.Meeting.findById(req.params.id)
            .populate('candidateId', 'firstName lastName email appliedRole')
            .populate('projectId', 'name')
            .populate('createdBy', 'name email');
        if (!meeting) {
...
const updateMeeting = async (req, res) => {
    try {
        const meeting = await Meeting_js_1.Meeting.findByIdAndUpdate(req.params.id, { $set: req.body }, { new: true });
        if (!meeting) {
            res.status(404).json({ success: false, message: 'Meeting not found' });
            return;
        }
        (0, socketHandler_js_1.getIO)()?.emit('meeting_updated', meeting);
        res.status(200).json({ success: true, meeting });
    }
    catch (error) {
 ...
 */
const sendCancellationEmails = async (meeting) => {
    const smtpUser = process.env.SMTP_USER;
    const smtpPass = process.env.SMTP_PASS;
    if (!smtpUser || !smtpPass) {
        logger_js_1.logger.warn('[MeetingController] SMTP credentials not set — skipping cancellation emails.');
        return;
    }
    ...
    if (attendeeEmails.length === 0) {
        logger_js_1.logger.info('[MeetingController] No attendees to notify for cancellation.');
        return;
    }
...
<body style="margin:0;padding:0;background:#f8fafc;font-family:'Figtree',Arial,sans-serif;">
  ...
  `;
    const emailPromises = attendeeEmails.map((email) => transporter.sendMail({
        from: `"EthicSecur HRMS" <${smtpUser}>`,
        to: email,
        subject: `❌ Meeting Cancelled: ${meeting.title}`,
        html: htmlBody,
    }).catch((err) => {
        logger_js_1.logger.warn(`[MeetingController] Failed to send cancellation email to ${email}: ${err.message}`);
    }));
    await Promise.allSettled(emailPromises);
    logger_js_1.logger.info(`[MeetingController] Cancellation emails sent to ${attendeeEmails.length} attendee(s).`);
};
/**
...
const deleteMeeting = async (req, res) => {
    try {
        ...
        try {
            const microsoftCredentials = await getOrgMicrosoftCredentials(orgId);
            await (0, teamsMeeting_service_js_1.cancelTeamsMeeting)(meeting.teamsMeetingId, meeting.organizer, microsoftCredentials);
        }
        catch (cancelErr) {
            logger_js_1.logger.warn(`[MeetingController] Teams-side cancellation failed (proceeding with local cancel): ${cancelErr.message}`);
        }
        meeting.status = 'CANCELLED';
        await meeting.save();
        (0, socketHandler_js_1.getIO)()?.emit('meeting_cancelled', { meetingId: meeting._id });
        // Send cancellation emails to all attendees
        sendCancellationEmails(meeting).catch((err) => {
            logger_js_1.logger.warn(`[MeetingController] Failed to send some cancellation emails: ${err.message}`);
        });
        res.status(200).json({ success: true, message: 'Meeting cancelled successfully' });
    }
    catch (error) {
        logger_js_1.logger.error('[MeetingController] deleteMeeting error', { error: error.message });
        res.status(500).json({ success: false, message: error.message });
    }
};
 ...
 */
const joinMeetingRedirect = async (req, res) => {
    try {
        const { id } = req.params;
        const meeting = await Meeting_js_1.Meeting.findById(id);
        if (!meeting) {
            res.status(404).send(getHtmlTemplate({
      ...
      `;
            res.status(403).send(getHtmlTemplate({
      ...
      `;
        ...
        logger_js_1.logger.error('[MeetingController] joinMeetingRedirect error', { error: error.message });
...
exports.joinMeetingRedirect = joinMeetingRedirect;
