...
exports.SessionPolicyService = void 0;
const SessionPolicy_js_1 = require("../models/SessionPolicy.js");
/**
...
class SessionPolicyService {
    ...
    static async getPolicy(organizationId) {
        ...
        let policy = await SessionPolicy_js_1.SessionPolicy.findOne({
            organizationId,
            isDefault: true,
            isActive: true,
        });
        if (!policy) {
            ...
            policy = await SessionPolicy_js_1.SessionPolicy.findOne({
                organizationId,
                isActive: true,
            });
        }
     ...
     */
    static async createPolicy(organizationId, data) {
        // If this is set as default, un-default others
        if (data.isDefault) {
            await SessionPolicy_js_1.SessionPolicy.updateMany({ organizationId, isDefault: true }, { isDefault: false });
        }
        return SessionPolicy_js_1.SessionPolicy.create({
            organizationId,
            ...data,
        });
    }
     ...
     */
    static async updatePolicy(policyId, organizationId, data) {
        if (data.isDefault) {
            await SessionPolicy_js_1.SessionPolicy.updateMany({ organizationId, isDefault: true, _id: { $ne: policyId } }, { isDefault: false });
        }
        return SessionPolicy_js_1.SessionPolicy.findOneAndUpdate({ _id: policyId, organizationId }, data, { new: true });
    }
    /**
     * Validate an IP against the policy whitelist/blacklist.
     */
    static validateIP(policy, ipAddress) {
        if (!policy.enforceIpRestriction) {
            return { allowed: true };
        }
        // Check blacklist first
        if (policy.ipBlacklist.length > 0) {
            const isBlacklisted = policy.ipBlacklist.some((pattern) => this.matchIPPattern(ipAddress, pattern));
            if (isBlacklisted) {
        ...
        if (policy.ipWhitelist.length > 0) {
            const isWhitelisted = policy.ipWhitelist.some((pattern) => this.matchIPPattern(ipAddress, pattern));
            if (!isWhitelisted) {
     ...
     */
    static validatePassword(policy, password) {
        const errors = [];
     ...
     */
    static async listPolicies(organizationId) {
        return SessionPolicy_js_1.SessionPolicy.find({ organizationId }).sort({ isDefault: -1, name: 1 });
    }
    /**
     * Delete a session policy (cannot delete the default).
     */
    static async deletePolicy(policyId, organizationId) {
        const policy = await SessionPolicy_js_1.SessionPolicy.findOne({ _id: policyId, organizationId });
        if (!policy)
            return false;
        if (policy.isDefault) {
     ...
     */
    static matchIPPattern(ip, pattern) {
        if (pattern.includes('/')) {
...
exports.SessionPolicyService = SessionPolicyService;
