...
exports.updateLifecycleDetails = exports.updateLifecycleStep = exports.createLifecycleTracker = exports.getLifecycleTrackers = void 0;
const mongoose_1 = __importDefault(require("mongoose"));
const EmployeeLifecycle_js_1 = require("../../models/EmployeeLifecycle.js");
const Employee_js_1 = require("../../models/Employee.js");
const getLifecycleTrackers = async (req, res, next) => {
    try {
        ...
        if (type)
            query.type = type;
        const trackers = await EmployeeLifecycle_js_1.EmployeeLifecycle.find(query)
...
exports.getLifecycleTrackers = getLifecycleTrackers;
const createLifecycleTracker = async (req, res, next) => {
    try {
        const orgId = req.user?.organizationId;
        ...
        const tracker = new EmployeeLifecycle_js_1.EmployeeLifecycle({
...
exports.createLifecycleTracker = createLifecycleTracker;
const updateLifecycleStep = async (req, res, next) => {
    ...
    try {
        ...
        const tracker = await EmployeeLifecycle_js_1.EmployeeLifecycle.findOne({ _id: trackerId, organizationId: orgId }).session(session);
        if (!tracker) {
        ...
        const step = tracker.steps.id(stepId);
        if (!step) {
        ...
        if (allCompleted) {
            ...
            if (tracker.type === 'PROBATION' && tracker.probationDetails?.isConfirmed) {
                await Employee_js_1.Employee.findOneAndUpdate(empQuery, { confirmationDate: new Date() }).session(session);
            }
            else if (tracker.type === 'PROMOTION' && tracker.promotionDetails) {
                await Employee_js_1.Employee.findOneAndUpdate(empQuery, {
...
exports.updateLifecycleStep = updateLifecycleStep;
const updateLifecycleDetails = async (req, res, next) => {
    try {
        const orgId = req.user?.organizationId;
        const { id } = req.params;
        const tracker = await EmployeeLifecycle_js_1.EmployeeLifecycle.findOneAndUpdate({ _id: id, organizationId: orgId }, req.body, { new: true });
        if (!tracker) {
            res.status(404).json({ message: 'Lifecycle tracker not found' });
            return;
        }
...
exports.updateLifecycleDetails = updateLifecycleDetails;
