...
const ConditionEvaluator_js_1 = require("./ConditionEvaluator.js");
const User_js_1 = require("../../models/User.js");
const Employee_js_1 = require("../../models/Employee.js");
const LeaveBalanceService_js_1 = require("../leave-engine/services/LeaveBalanceService.js");
...
class WorkflowRunner {
     ...
     */
    static async triggerWorkflow(organizationId, triggerEvent, refModel, refId, session) {
     ...
     */
    static async buildContext(instance, session) {
        const context = {};
        let refDoc = null;
        let employeeId = null;
        let submitterUserId = null;
        try {
            const RefModel = mongoose_1.default.model(instance.refModel);
            refDoc = await RefModel.findOne({ _id: instance.refId, organizationId: instance.organizationId }).session(session || null);
        }
        catch (err) {
            logger_js_1.logger.error(`[WorkflowRunner] Failed to fetch refDoc: ${instance.refModel} / ${instance.refId}`, err);
        }
        if (!refDoc) {
            return context;
        }
        ...
        if (employeeId) {
            employeeDoc = await Employee_js_1.Employee.findOne({ _id: employeeId, organizationId: instance.organizationId }).session(session || null);
        }
        else if (submitterUserId) {
            const user = await User_js_1.User.findOne({ _id: submitterUserId, organizationId: instance.organizationId }).session(session || null);
            if (user) {
                employeeDoc = await Employee_js_1.Employee.findOne({ email: user.email, organizationId: instance.organizationId }).session(session || null);
            }
        }
        if (employeeDoc) {
        ...
        if (submitterUserId) {
            submitterUser = await User_js_1.User.findOne({ _id: submitterUserId, organizationId: instance.organizationId }).session(session || null);
        }
        else if (employeeDoc) {
            submitterUser = await User_js_1.User.findOne({ email: employeeDoc.email, organizationId: instance.organizationId }).session(session || null);
        }
        if (submitterUser) {
     ...
     */
    static async syncReferenceDocument(instance, status, actingUserId, session) {
        const RefModel = mongoose_1.default.model(instance.refModel);
        const refDoc = await RefModel.findOne({ _id: instance.refId, organizationId: instance.organizationId }).session(session || null);
        if (!refDoc) {
            logger_js_1.logger.warn(`[WorkflowRunner] Reference doc not found for syncing: ${instance.refModel} (${instance.refId})`);
            return;
        }
        refDoc.status = status;
        if (actingUserId) {
            refDoc.approvedBy = new mongoose_1.default.Types.ObjectId(actingUserId);
        }
        if (status === 'APPROVED') {
            if (instance.refModel === 'Leave') {
                ...
                if (refDoc.leaveType !== 'WFH' && refDoc.leaveType !== 'Unpaid Leave') {
                    const deducted = await LeaveBalanceService_js_1.LeaveBalanceService.deductBalance(instance.organizationId.toString(), refDoc.employeeId.toString(), refDoc.leaveType, refDoc.totalDays, session);
                    if (!deducted) {
                    ...
                    await LeaveBalanceService_js_1.LeaveBalanceService.deductBalance(instance.organizationId.toString(), refDoc.employeeId.toString(), 'WFH', refDoc.totalDays, session);
                }
            }
        ...
        logger_js_1.logger.info(`[WorkflowRunner] Synced ${instance.refModel} (${instance.refId}) status to ${status}.`);
    }
    /**
     * Advances a workflow instance based on context and user actions.
     */
    static async advance(instanceId, contextOverride, userAction, actingUser, comments, session) {
        // Acquire a state lock on the instance to prevent concurrent race conditions
        const instance = await WorkflowInstance_js_1.WorkflowInstance.findOneAndUpdate({ _id: instanceId, status: 'ACTIVE' }, { $set: { updatedAt: new Date() } }, { session, new: true });
        if (!instance) {
            logger_js_1.logger.warn(`[WorkflowRunner] Workflow instance ${instanceId} is not active or does not exist.`);
            return null;
        }
        const template = await WorkflowTemplate_js_1.WorkflowTemplate.findById(instance.workflowTemplateId).session(session || null);
        if (!template) {
        ...
        while (currentNode && jumps < 50) {
            ...
            if (currentNode.type === 'APPROVAL') {
                if (!userAction) {
                    ...
                    if (actingUser.role === 'ADMIN') {
                        ...
                        if (submitterEmpId) {
                            const submitterEmpDoc = await Employee_js_1.Employee.findById(submitterEmpId).session(session || null);
                            if (submitterEmpDoc && submitterEmpDoc.primaryManagerId) {
                                const actingUserDoc = await User_js_1.User.findById(actingUser.id).session(session || null);
                                let actingEmployeeId = actingUserDoc?.employeeId;
                                if (!actingEmployeeId && actingUser.email) {
                                    const actingEmpDoc = await Employee_js_1.Employee.findOne({ email: actingUser.email, organizationId: instance.organizationId }).session(session || null);
                    ...
                    if (activeLog) {
                        ...
                        activeLog.approverUserId = new mongoose_1.default.Types.ObjectId(actingUser.id);
                        ...
                        instance.history.push({
                            nodeId: currentNode.id,
                            nodeName: currentNode.name,
                            approverUserId: new mongoose_1.default.Types.ObjectId(actingUser.id),
            ...
            if (currentNode.type === 'CONDITION') {
                ...
                if (conditionField && conditionOperator) {
                    result = ConditionEvaluator_js_1.ConditionEvaluator.evaluate(context, conditionField, conditionOperator, conditionValue);
                }
            ...
            if (currentNode.type === 'NOTIFICATION') {
                ...
                const nextNodeId = currentNode.config?.nextNodes instanceof Map
                    ? currentNode.config.nextNodes.get('true')
