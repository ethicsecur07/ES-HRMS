...
const TemporaryGrant_js_1 = require("../models/TemporaryGrant.js");
const policyEvaluator_js_1 = require("../utils/policyEvaluator.js");
...
const rbacGuard = (moduleCode, action) => {
    return async (req, res, next) => {
        try {
            ...
            const activeGrant = await TemporaryGrant_js_1.TemporaryGrant.findOne({
            ...
            if (activeGrant && activeGrant.actions[action] === true) {
            ...
            const hasUnrestrictedGrant = grantingPermissions.some((p) => !p.policyCondition || (Array.isArray(p.policyCondition) && p.policyCondition.length === 0));
            req.checkPolicyCondition = (resource) => {
                ...
                return (0, policyEvaluator_js_1.evaluatePolicy)(policies, user, resource);
            };
            setupSanitizedResponse(req, res);
            next();
        }
 ...
 */
function setupSanitizedResponse(req, res) {
    const originalJson = res.json;
    res.jsonSanitized = function (data) {
