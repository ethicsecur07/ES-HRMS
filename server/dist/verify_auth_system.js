...
const createMockReq = (body, cookies = {}, headers = {}, ip = '127.0.0.1') => ({
    body,
    cookies,
    headers: {
...
async function runTests() {
    ...
    try {
        await mongoose_1.default.connection.collection('organizations').dropIndex('adminEmail_1');
    ...
    await User_js_1.User.deleteMany({ email: testEmail });
    ...
    await UserSession_js_1.UserSession.deleteMany({ userId: { $exists: true } }); // Clean sessions during test runs
    await LoginEvent_js_1.LoginEvent.deleteMany({ email: testEmail });
    ...
    const bcryptHash = await bcryptjs_1.default.hash(testPassword, 12);
    const testUser = await User_js_1.User.create({
        name: 'Auth Test User',
        email: testEmail,
        password: bcryptHash,
        role: 'ADMIN',
    ...
    await (0, auth_controller_js_1.login)(req1, res1);
    ...
    const updatedUser = await User_js_1.User.findById(testUser._id).select('+password');
    ...
    await (0, auth_controller_js_1.login)(req2, res2);
    ...
    await UserSession_js_1.UserSession.deleteMany({ userId: testUser._id });
    ...
    for (let i = 1; i <= 4; i++) {
        ...
        await (0, auth_controller_js_1.login)(req, res);
    ...
    const sessions = await UserSession_js_1.UserSession.find({ userId: testUser._id }).sort({ createdAt: 1 });
    ...
    await (0, auth_controller_js_1.refreshToken)(reqRefresh1, resRefresh1);
    ...
    await (0, auth_controller_js_1.refreshToken)(reqReplay, resReplay);
    ...
    const allSessionsAfterReplay = await UserSession_js_1.UserSession.find({ userId: testUser._id });
    ...
    await LoginEvent_js_1.LoginEvent.create({
        email: testEmail,
        status: 'SUCCESS',
        ipAddress: '8.8.8.8',
    ...
    const riskResult = await LoginRiskService_js_1.LoginRiskService.evaluateRisk({
        email: testEmail,
        ipAddress: '1.1.1.1',
        userAgent: 'Mozilla/5.5',
