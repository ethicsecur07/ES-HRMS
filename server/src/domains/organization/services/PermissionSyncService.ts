...
export class PermissionSyncService {
   ...
   */
  static async syncForTenant(orgId: mongoose.Types.ObjectId, session?: mongoose.ClientSession, force = false): Promise<void> {
    try {
      logger.info(`🔑 Synchronizing role permissions for organization: ${orgId} (force reset: ${force})`);

      let roles = await Role.find({ organizationId: orgId }).session(session || null);
      const rolesMissingSlug = roles.filter((role) => !role.slug);
      if (rolesMissingSlug.length > 0) {
        await Role.bulkWrite(
          rolesMissingSlug.map((role) => ({
            updateOne: {
              filter: { _id: role._id },
        ...
        roles = await Role.find({ organizationId: orgId }).session(session || null);
      }
      ...
      const createdRoles = await Role.find({ organizationId: orgId }).session(session || null);
      ...
      await Permission.bulkWrite(operations, { session });

      logger.info(`✅ Role permissions successfully synchronized for org ${orgId}.`);
    } catch (error) {
      logger.error('❌ Failed to synchronize role permissions:', { error });
      throw error;
    }
  }
}
