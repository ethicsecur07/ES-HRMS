...
export const taskVisibilityMiddleware = async (req: AuthRequest, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    if (role === 'EMPLOYEE') {
      ...
      if (!employeeId) {
        (req as any).taskFilter = { employeeId: new User()._id }; // Matches nothing
        return next();
      }
