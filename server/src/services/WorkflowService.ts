...
import { Approval, IApproval, IApprovalStep } from '../models/Approval.js';
...
export class WorkflowService {
   ...
   */
  private static async isEmployeeOnLeave(employeeId: mongoose.Types.ObjectId | string, orgId: any): Promise<boolean> {
   ...
   */
  public static async isManagerAvailable(managerEmployeeId: mongoose.Types.ObjectId | string, orgId: any): Promise<boolean> {
    if (!managerEmployeeId) return false;
    
    // 1. Check if employee is active
    const emp = await Employee.findOne({ _id: managerEmployeeId, organizationId: orgId });
    ...
    const user = await User.findOne({ employeeId: managerEmployeeId, organizationId: orgId });
   ...
   */
  public static async getAvailableHRs(orgId: any): Promise<string[]> {
    ...
    const availableHrUserIds: string[] = [];
  ...
  public static async initiateApproval(
    ...
    const workflowChain: IApprovalStep[] = [];
    ...
    let managerUserId: mongoose.Types.ObjectId | undefined = undefined;
    ...
    if (employee && employee.primaryManagerId) {
      ...
      if (isAvailable) {
        const mUser = await User.findOne({ employeeId: employee.primaryManagerId, organizationId });
        if (mUser) {
          managerAvailable = true;
          managerUserId = mUser._id as mongoose.Types.ObjectId;
          workflowChain.push({
            roleCode: 'MANAGER',
            status: 'PENDING',
    ...
    await createAuditLog(
      ...
      'WORKFLOW',
      approval.id,
      `Initiated approval workflow for ${refModel}. Current step: ${firstStep.roleCode}`,
      organizationId
    );
  ...
  public static async processStep(
    ...
    try {
      ...
      currentStep.actionById = new mongoose.Types.ObjectId(userId);
      ...
      if (action === 'REJECTED') {
        ...
        const refModel = mongoose.model(approval.refModel);
        await refModel.findOneAndUpdate(
          { _id: approval.refId, organizationId: approval.organizationId },
          { status: 'REJECTED', rejectionReason: comments || 'Rejected by workflow' }
        ).session(session);
        ...
        await createAuditLog(
          ...
          'WORKFLOW',
          approval.id,
          `Workflow rejected by ${userRole}. comments: ${comments || 'none'}`,
        ...
        if (shouldAutoComplete) {
          ...
          const refModel = mongoose.model(approval.refModel);
          const doc: any = await refModel.findOne({ _id: approval.refId, organizationId: approval.organizationId }).session(session);
          ...
          await createAuditLog(
            ...
            'WORKFLOW',
            approval.id,
            `Workflow step approved by ${userRole}. Advancing to ${nextStep.roleCode}`,
            approval.organizationId
          );
