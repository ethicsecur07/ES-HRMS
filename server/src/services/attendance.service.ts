...
export class AttendanceService {
  static async getTodayAttendance(organizationId: string, userId?: string, role?: string, email?: string) {
    const today = new Date().toISOString().split('T')[0];
    const query: any = { date: today, organizationId };
  ...
  static async getAllAttendance(organizationId: string, userId?: string, role?: string, email?: string) {
  ...
  static async checkIn(organizationId: string, employeeId: string, email: string, ipAddress: string, deviceInfo: any, overrideReason?: string) {
    const today = new Date().toISOString().split('T')[0];
    
    const existing = await Attendance.findOne({ employeeId, date: today, organizationId });
    if (existing) throw new Error('Attendance already recorded for today');
    ...
    try {
      if (currentHour < 9 || (currentHour === 9 && currentMinute <= 35)) {
        ...
        const emp = await Employee.findOne({ _id: employeeId, organizationId }).session(session);
  ...
  static async checkOut(organizationId: string, attendanceId: string, email: string, taskReportId?: string) {
    const session = await mongoose.startSession();
    session.startTransaction();
    
    try {
      const attendance = await Attendance.findOne({ _id: attendanceId, organizationId }).session(session);
      ...
      if (currentHour >= 18 || (currentHour === 17 && currentMinute >= 40)) {
        ...
        const emp = await Employee.findOne({ _id: attendance.employeeId, organizationId }).session(session);
  ...
  static async updateAttendance(organizationId: string, attendanceId: string, email: string, loginTime?: string, logoutTime?: string, status?: string) {
    const attendance = await Attendance.findOne({ _id: attendanceId, organizationId });
    if (!attendance) throw new Error('Attendance record not found');
