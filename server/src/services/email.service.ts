...
const {
  ...
  SMTP_PORT = '587',
  SMTP_USER = 'suseendrakumar@ethicsecur.co.in',
  SMTP_PASS,
  TENANT_ID,
  CLIENT_ID,
  CLIENT_SECRET
...
export const sendEmail = async (options: SendEmailOptions): Promise<void> => {
  try {
    ...
    if (hasSmtpPassword) {
      logger.info(`[EmailService] Using password-based SMTP AUTH for ${SMTP_USER}...`);
      ...
      try {
        ...
        if (graphResponse.ok) {
          logger.info(`[EmailService] Email sent successfully via Microsoft Graph API.`);
          isGraphSent = true;
        } else {
          const errorText = await graphResponse.text();
          logger.warn(`[EmailService] Microsoft Graph API sendMail returned status ${graphResponse.status}: ${errorText}. Falling back to Nodemailer SMTP OAuth2...`);
        }
      } catch (graphError: any) {
        logger.warn(`[EmailService] Microsoft Graph API sendMail failed: ${graphError.message || graphError}. Proceeding to fallback SMTP...`);
      }
      ...
      logger.warn('[EmailService] Microsoft Azure AD OAuth2 credentials are unconfigured or incomplete in .env. Operating in DRY-RUN mode.');
      transporter = {
        sendMail: async (mailOptions: any) => {
          logger.info(`[EmailService DRY-RUN] Dispatching to ${mailOptions.to}. Subject: ${mailOptions.subject}. (Transporter operates in dry-run)`);
    ...
    if (!isGraphSent) {
      ...
      logger.info(`[EmailService] Email sent successfully via SMTP. Message ID: ${info.messageId}`);
    }
  } catch (error: any) {
    logger.error('[EmailService] Failed to send email via Microsoft OAuth2 Services', { error: error.message || error, to: options.to });
    throw new Error(
