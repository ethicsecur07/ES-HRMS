 ...
 */
export const generateOfferLetterPdf = async (params: OfferLetterParams): Promise<Buffer> => {
  return new Promise((resolve, reject) => {
    try {
      ...
      const logoPath = path.resolve(process.cwd(), '../client/src/assets/ES_Logo.png');
...
We look forward to welcoming you to our team and are confident that your skills and dedication will make a valuable contribution to ES EthicSecur SofTec Pvt Ltd. Please confirm your acceptance of this offer by signing and returning a copy of this letter.`;
      ...
      const signImagePath = path.resolve(process.cwd(), '../client/src/assets/ES_Sign.png');
      ...
      logger.error('[OfferLetterPdfService] Failed to generate PDF', { error });
      reject(error);
    }
  });
};
